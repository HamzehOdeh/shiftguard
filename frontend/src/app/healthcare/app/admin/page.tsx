...
const AUDIT_LOG = [
  { time: '2h ago', action: 'VIOLATION_FIXED', actor: 'System (Auto)', target: 'Dr. Patel', detail: '80h limit approaching — removed Friday night shift' },
  { time: '3h ago', action: 'SWAP_APPROVED', actor: 'Dr. Kim', target: 'Dr. Santos', detail: 'Night swap — both ACGME-safe after swap' },
  { time: '4h ago', action: 'PTO_APPROVED', actor: 'System', target: 'RN Sarah Chen', detail: 'Auto-approved — coverage maintained' },
...
export default function AdminPage() {
  return (
    <div className="px-5 py-6 space-y-6">
      ...
      {/* Penalty exposure */}
      <div className="bg-surface-raised border border-white/[0.06] border-t-2 border-t-red-500 rounded-2xl p-5 shadow-elevation-2">
      ...
      <div>
        ...
        <div className="space-y-3">
          {VIOLATIONS.map((v, i) => (
            <div key={i} className={`bg-surface-raised border rounded-2xl p-4 shadow-elevation-1 ${
        ...
        <div className="space-y-2">
          {AUDIT_LOG.map((log, i) => (
            <div key={i} className="bg-surface-raised border border-white/[0.06] rounded-xl px-4 py-3 shadow-elevation-1">
              ...
              <p className="text-xs text-gray-300">{log.actor} → {log.target}</p>
